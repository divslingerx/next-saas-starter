...
export const metafield = createTable(
  ...
  (table) => [
    index("metafield_entity_idx").on(table.entityType, table.entityId),
    index("metafield_namespace_key_idx").on(table.namespace, table.key),
    index("metafield_public_idx").on(table.isPublic),
  ],
);
...
export const tagAssignment = createTable(
  ...
  (table) => ({
    entityIdx: index("tag_assignment_entity_idx").on(
      table.entityType,
      table.entityId,
    ),
